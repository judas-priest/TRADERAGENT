...
class SMCSignal:
    ...
    def __repr__(self) -> str:
        return (f"SMCSignal({self.direction.value.upper()}, "
...
class EntrySignalGenerator:
    ...
    def __init__(
        ...
        logger.info(
            "EntrySignalGenerator initialized",
    ...
    def _detect_pin_bar_patterns(self, df: pd.DataFrame) -> None:
        ...
        for i in range(len(df)):
            ...
            if (lower_wick > candle_range * 0.6 and  # Long lower wick
                body_size < candle_range * 0.4 and   # Small body
                upper_wick < candle_range * 0.2):    # Small upper wick
            ...
            elif (upper_wick > candle_range * 0.6 and  # Long upper wick
                  body_size < candle_range * 0.4 and   # Small body
                  lower_wick < candle_range * 0.2):    # Small lower wick
    ...
    def _calculate_pin_bar_quality(self, candle, is_bullish: bool) -> float:
        ...
        body_center = (max(candle["open"], candle["close"]) + min(candle["open"], candle["close"])) / 2
        if is_bullish:
        ...
        for i in range(1, len(df)):
            ...
            if (curr["high"] <= prev["high"] and
                ...
                if quality >= 50:
                    ...
                    logger.debug(f"{direction} Inside Bar detected at index {i}, quality={quality:.1f}")
    ...
    def get_signals_summary(self) -> dict:
        ...
        return {
            ...
            "engulfing": len([p for p in self.detected_patterns if p.pattern_type == PatternType.ENGULFING]),
            "pin_bar": len([p for p in self.detected_patterns if p.pattern_type == PatternType.PIN_BAR]),
            "inside_bar": len([p for p in self.detected_patterns if p.pattern_type == PatternType.INSIDE_BAR]),
            "long_signals": len([s for s in self.generated_signals if s.direction == SignalDirection.LONG]),
            "short_signals": len([s for s in self.generated_signals if s.direction == SignalDirection.SHORT]),
            "avg_confidence": np.mean([s.confidence for s in self.generated_signals]) if self.generated_signals else 0.0,
            "avg_rr_ratio": np.mean([s.risk_reward_ratio for s in self.generated_signals]) if self.generated_signals else 0.0
        }
