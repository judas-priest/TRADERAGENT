...
class ConfluenceZoneAnalyzer:
    ...
    def _detect_fair_value_gaps(self, df: pd.DataFrame) -> None:
        ...
        for i in range(len(df) - 2):
            ...
            if candle0["high"] < candle2["low"]:
                ...
                existing = any(
                    fvg.candle1_index == i and fvg.is_bullish
            ...
            # Check for bearish FVG
                ...
                existing = any(
                    fvg.candle1_index == i and not fvg.is_bullish
                    for fvg in self.fair_value_gaps
                )
    ...
    def _cleanup_zones(self) -> None:
        ...
        self.order_blocks = [
            ...
            if ob.status == ZoneStatus.ACTIVE or
            (ob.invalidated_at and (datetime.now() - ob.invalidated_at).days < 1)
        ][:self.max_active_zones]
        
        self.fair_value_gaps = [
            fvg for fvg in self.fair_value_gaps
            if fvg.status in [ZoneStatus.ACTIVE, ZoneStatus.PARTIAL_FILL] or
            (fvg.filled_at and (datetime.now() - fvg.filled_at).days < 1)
        ][:self.max_active_zones]
