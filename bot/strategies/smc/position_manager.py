...
class PositionManager:
    ...
    def validate_position_risk(
        ...
        if position_value > self.max_position_size:
            return False, f"Position size {float(position_value):.2f} exceeds max {float(self.max_position_size)}"
        ...
        if total_exposure > max_total_exposure:
            return False, f"Total exposure {float(total_exposure):.2f} exceeds max {float(max_total_exposure)}"
