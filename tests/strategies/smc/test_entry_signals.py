...
from bot.strategies.smc.entry_signals import (
    EntrySignalGenerator, PatternType, SignalDirection, SMCSignal, PriceActionPattern
)
...
class TestEntrySignalGenerator(unittest.TestCase):
    ...
    def test_engulfing_detection(self):
        ...
        engulfing_patterns = [p for p in self.generator.detected_patterns 
                             if p.pattern_type == PatternType.ENGULFING]
    ...
    def test_pin_bar_detection(self):
        ...
        pin_bars = [p for p in self.generator.detected_patterns 
                   if p.pattern_type == PatternType.PIN_BAR]
    ...
    def test_inside_bar_detection(self):
        ...
        inside_bars = [p for p in self.generator.detected_patterns 
                      if p.pattern_type == PatternType.INSIDE_BAR]
